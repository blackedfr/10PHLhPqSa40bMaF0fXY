...
local BringLocations = {
    ...
    ["cell"] = CFrame.new(-798.214172, -39.5528908, -845.649902, 0.99030453, 1.2909978e-08, -0.138913214, -6.15148654e-10, 1, 8.85502018e-08, 0.138913214, -8.76062174e-08, 0.99030453),	
}
...
local SetupsTable = {
    ...
    Cell = {
		Origin = CFrame.new(-790.607056, -39.5528908, -842.384155, 0.999454916, 1.34697165e-09, -0.033013273, 1.87065452e-09, 1, 9.7433734e-08, 0.033013273, -9.74423813e-08, 0.999454916),
		ZMultiplier = 2,
		XMultiplier = -2,
		PerRow = 5,
		Rows = 4,
	...
	Connections["OnChat"] = game.ReplicatedStorage.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(data)
		...
		if Host and not Crashed and Variables["Player"].Character and Variables["Player"].Character:FindFirstChild("HumanoidRootPart") and Variables["Player"].Character:FindFirstChild("Humanoid") and Variables["Player"].Character.Humanoid.Health > 0 then
			...
			elseif Message == ".setup train" then
				Setup("Train")
            elseif Message == ".setup cell" then
				Setup("Cell")
